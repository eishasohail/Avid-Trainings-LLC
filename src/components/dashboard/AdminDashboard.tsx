...
import { Users, UserCheck, Shield, Activity, TrendingUp, TrendingDown, ChevronRight, UserPlus, CheckCircle, Award, LayoutDashboard } from "lucide-react";
...
export default function AdminDashboard({ user }: { user: AuthUser }) {
  ...
  const stats = [
    { label: "Total Users", value: "24,842", icon: Users, color: "bg-blue-500", trend: "+12%" },
    { label: "Avg. Completion", value: "78%", icon: Activity, color: "bg-[#00685f]", trend: "+0.1%" },
    { label: "Total Courses", value: "4", icon: CheckCircle, color: "bg-emerald-500", trend: "+5.1%" },
    { label: "Total Learners", value: "3", icon: Shield, color: "bg-[#00685f]", trend: "+2%" },
  ];
  ...
  return (
    <div className="space-y-10 sm:space-y-12">
      ...
      <header className="animate-fade-in-up">
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-[#191c1e] tracking-tighter">Dashboard</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in-up stagger-2 pb-10">
        ...
        <section className="lg:col-span-2 bg-white rounded-[48px] border border-[#bcc9c6]/40 p-10 shadow-sm space-y-10 group hover:border-[#00685f]/30 transition-all duration-500">
           <div className="flex flex-col sm:flex-row items-center justify-between border-b border-[#bcc9c6]/10 pb-8 gap-6">
              <h3 className="text-xl sm:text-2xl font-black text-[#191c1e] tracking-tighter flex items-center gap-4">
                 <TrendingUp className="w-8 h-8 text-[#00685f]" /> Enrollment Trend
              </h3>
           </div>
        ...
        <section className="bg-white rounded-[48px] border border-[#bcc9c6]/40 p-10 shadow-sm space-y-12 group hover:border-[#00685f]/30 transition-all duration-500 flex flex-col items-center text-center">
           <h3 className="text-xl sm:text-2xl font-black text-[#191c1e] tracking-tight">Course Status</h3>
           ...
           <div className="relative w-56 h-56 sm:w-64 sm:h-64 group-hover:scale-105 transition-transform duration-1000">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                 ...
                 <circle cx="50" cy="50" r="40" fill="none" stroke="#131b2e" strokeWidth="12" strokeDasharray="251.2" strokeDashoffset="180" className="animate-[dash_2.5s_ease-out_forwards]" />
                 <circle cx="50" cy="50" r="40" fill="none" stroke="#bcc9c6" strokeWidth="12" strokeDasharray="251.2" strokeDashoffset="220" className="animate-[dash_3s_ease-out_forwards]" />
              </svg>
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                 <span className="text-4xl font-black text-[#191c1e] tracking-tight">72%</span>
                 <span className="text-[10px] font-black uppercase tracking-[0.15em] text-[#6d7a77]">Completion</span>
              </div>
           </div>

           <div className="w-full space-y-6 pt-4">
              {[
                { label: "Completed", val: 75, color: "bg-[#00685f]" },
                { label: "In Progress", val: 18, color: "bg-[#131b2e]" },
                { label: "Not Started", val: 7, color: "bg-[#bcc9c6]" }
              ].map((item, i) => (
      ...
      <section className="bg-white rounded-[40px] border border-[#bcc9c6]/40 p-8 sm:p-12 shadow-sm space-y-12 group hover:border-[#00685f]/30 transition-all duration-500">
         <div className="flex justify-between items-center border-b border-[#bcc9c6]/10 pb-8">
            <h3 className="text-2xl font-black text-[#191c1e] tracking-tighter">Recent Users</h3>
            <button 
              onClick={() => router.push('/dashboard/admin')}
              className="text-[11px] font-black uppercase tracking-widest text-[#00685f] hover:tracking-[0.2em] transition-all flex items-center gap-2 group/btn"
            >
               View All <ChevronRight className="w-4 h-4 transition-transform group-hover/btn:translate-x-1" />
            </button>
         </div>
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              { name: "Fatima Sohail", role: "Creator", users: 124, status: "Active" },
              { name: "Ahmed Khan", role: "Learner", users: 42, status: "Pending" },
              { name: "Eisha Fakhir", role: "Admin", users: 856, status: "Primary" },
              { name: "Omar Sheikh", role: "Learner", users: 24, status: "Active" }
            ].map((u, i) => (
